...
The web version ships a private, per-reader dashboard with server-backed accounts, auto-saved reading progress, and a daily streak, synced across devices through the `api/auth` and `api/progress` endpoints and a Neon Postgres database.

| Feature | What it does |
|---|---|
| **Accounts** | Sign up / log in with email + password, or **Continue with Google**. Passwords are salted and hashed with scrypt; sessions live in an HttpOnly cookie and persist across visits. |
